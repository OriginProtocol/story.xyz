...
export function formatCurrencyConditional(
  value: string,
  threshold: number,
  decimalsBeforeThreshold: number,
  decimalsAfterThreshold: number
) {
  if (value === "") {
    return "0.00";
...
export function formatCurrencyAbbreviated(num: number, decimalDigits: number) {
  const lookup = [
    { value: 1, symbol: "" },
    { value: 1e3, symbol: "k" },
    { value: 1e6, symbol: "M" },
...
export function aprToApy(apr: number, aprDays: number) {
 ...
 */
export function truncateDecimals(value: string, decimals = 6) {
...
export function checkValidInputForCoin(amount: string, coin: string) {
  if (amount === "") {
    amount = "0.00";
  }
