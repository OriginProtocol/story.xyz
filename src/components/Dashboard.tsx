...
type infoMapType = [string, string, number, number];

export default function Dashboard({ circulatingOgn, ognPrice }: Props) {
      ...
      <section className="max-w-screen-xl mx-auto px-9 sm:py-16">
        <div className="flex flex-col md:flex-row justify-between">
          <div className="flex flex-col text-white w-full">
            <div className="flex items-start justify-between">
              <h3
                className={`text-[1.5rem] leading-[1.85rem] w-[75%] sm:w-[100%] sm:text-[3rem] md:leading-[4rem] font-bold ${styles.header}`}
              >
              ...
              {infoMap.map(([label, unit, value, decimals]) => (
                <div key={label}>
                  ...
                  <p className="text-[1.5rem] sm:text-[2rem] font-bold mb-2">
                    {unit}
                    {value ? formatCurrency(value.toString(), decimals) : ""}
                  </p>
                </div>
              ))}
            </div>
        ...
        <div className="w-full text-white flex flex-col items-center pt-48 pb-20 relative">
          ...
          <a
            ...
            className="cursor-pointer w-full sm:hidden text-center px-10 py-4 mb-16 mt-6 shadow rounded-full bg-gradient-to-r from-[#FA00FF] to-[#FED3AB] relative z-20"
            target="_blank"
            rel="noreferrer"
          >
            Earn Rewards
          ...
          <a
            href="https://app.story.xyz/stake"
            className="cursor-pointer hidden md:block w-64 text-center px-10 py-4 mt-6 shadow rounded-full bg-gradient-to-r from-[#FA00FF] to-[#FED3AB] relative z-20"
            target="_blank"
            rel="noreferrer"
          >
            Earn Rewards
          ...
          <div
            className="absolute"
            style={{ right: "calc(50% - 62rem)", top: "24rem" }}
          >
            <Image
